...
def draft_from(payload: dict[str, Any], action: dict[str, Any], private_dir: Path | None = None) -> dict[str, Any]:
    ...
    if private_dir is not None:
        ...
        _write_private_json(Path(private_raw_path), raw_payload)
    return {
...
def main() -> int:
    ...
    ap.add_argument('--write-raw-private', action='store_true', help='Write private raw drafts with 0600 permissions.')
    args = ap.parse_args()
    private_dir = None
    ...
    if not queue.exists():
        ...
        for p in rows:
            ...
            if action_hint not in SAFE_ACTIONS:
                skipped['unsafe_or_manual_action'] += 1
                continue
            d = draft_from(p, a, private_dir)
            drafts.append(d)
        ...
        report['counts'] = {
            'drafts': len(drafts),
            'private_raw_files': private_raw_files,
            'skipped': dict(skipped),
            'by_route': dict(by_route),
            'by_risk': dict(by_risk),
            'by_namespace': {digest_text(ns): count for ns, count in by_ns.items()},
        }
        if not drafts:
    ...
    if report['findings']:
        for f in report['findings']:
            lines.append(f"- [{f.get('severity','P?')}] {f.get('title')} `{json.dumps({k:v for k,v in f.items() if k not in ['severity','title']}, ensure_ascii=False)}`")
    else:
