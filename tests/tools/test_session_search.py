...
class TestUserScopeIsolation:
    def _seed_private_sessions(self, db):
        db.create_session("tg_alice", source="telegram", user_id="alice")
        db.append_message("tg_alice", role="user", content="sharedneedle alice private telegram")
        db.create_session("tg_bob", source="telegram", user_id="bob")
        db.append_message("tg_bob", role="user", content="sharedneedle bob private telegram")
        db.create_session("cli_local", source="cli")
        db.append_message("cli_local", role="user", content="sharedneedle cli local")

    def test_discovery_filters_by_gateway_user_and_source(self, db):
        self._seed_private_sessions(db)
        result = json.loads(session_search(
            query="sharedneedle", db=db, user_id="alice", source="telegram"
        ))
        assert result["success"] is True
        assert [r["session_id"] for r in result["results"]] == ["tg_alice"]

    def test_browse_filters_by_gateway_user_and_source(self, db):
        self._seed_private_sessions(db)
        result = json.loads(session_search(db=db, user_id="bob", source="telegram"))
        sids = [r["session_id"] for r in result["results"]]
        assert sids == ["tg_bob"]

    def test_cli_scope_does_not_hide_cli_sessions(self, db):
        self._seed_private_sessions(db)
        result = json.loads(session_search(query="sharedneedle", db=db, source="cli"))
        assert result["success"] is True
        assert [r["session_id"] for r in result["results"]] == ["cli_local"]

    def test_no_scope_preserves_admin_style_all_session_search(self, db):
        self._seed_private_sessions(db)
