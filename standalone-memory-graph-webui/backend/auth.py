...
Namespace: admin users get namespace="" (see all), regular users get their namespace.
"""
...
# ---------------------------------------------------------------------------
# Network auth enforcement
# ---------------------------------------------------------------------------

def enforce_network_auth(host: str = "127.0.0.1") -> None:
    """Refuse to start if host is network-reachable and no users exist."""
    ...
    if not users:
        raise RuntimeError(
            f"Host '{host}' is network-reachable but no users exist. "
            "Create at least one user before starting on a non-localhost host."
        )
