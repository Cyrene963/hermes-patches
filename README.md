...
> 一句话：这是一个**召回/隔离/拦截层已验证可靠的"外置大脑"**。自动写入（"越用越聪明"）现在**默认开启**，但每条写入都要过一个 **LLM 事实分类器 + readback 验证**，并且 **fail-closed**——LLM 不可达时一条都不写（宁可不学，绝不污染，这是 2026-05-25 事故的教训）。我们只标可复现验证过的东西，不把愿景写成成果。
