...
7. For complex repo work, use the `agentic-workflow-synthesis` + `deep-work-orchestration` combo: test the target path, compare against the source idea / prior version, simulate real usage, and state fidelity boundaries explicitly before declaring success.
...
| `individual/0012-fix-openrouter-key-routing-and-516-canary.patch` | provider/runtime routing | Default hotfix patch: OpenRouter auto/runtime paths use `OPENROUTER_API_KEY` only, never generic `OPENAI_API_KEY`; adds 516 anomaly guard regressions and a targeted live canary via `HERMES_516_CANARY_PROVIDER` / `HERMES_516_CANARY_MODEL`. | Focused runtime tests: `tests/hermes_cli/test_openrouter_key_routing.py`, `tests/hermes_cli/test_runtime_provider_resolution.py`, `tests/run_agent/test_516_anomaly_guard.py`, `tests/run_agent/test_chat_completions_516_anomaly_live.py`. Current HKU live route resolves but times out upstream. | `git apply -R` if applied. |
